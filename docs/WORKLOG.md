...
Last updated: 2026-07-21 (Claude / Opus) — build 14, voluntary mid-turn hand refill.
...
- **First-player advantage — proposed rules change, awaiting Nick's go-ahead (raised 2026-07-21).** `startRound()` hardcodes `state.currentPlayer = 0` ([app/index.html:2309](../app/index.html#L2309)), so **P1 starts every round of every game**. A round is a race (it ends the instant someone clears their HOLD pile, and the loser keeps theirs as points), so starting first is a structural edge. Nick's proposed fix: rotate the starter each round — one line, `state.currentPlayer = (state.round - 1) % state.players.length`. Data so far: **P1 has won 4 of 5 games, which is NOT significant** (18.8% by chance). Recommendation is to make the change on the structural argument alone — it is standard card-game practice, has no downside, and removes a confound from future data. Wrinkle: 3 players over 4 rounds gives P1 two starts.
- **Record per-round winners** — the cheapest way to settle the above. 5 games produced 5 data points but contained 15 rounds; one extra field triples the power using games already played.
- **Richer telemetry needed to diagnose the above.** Current stats are lifetime counters — volume, not shape. Highest value additions: per-round timestamps + turn counts, HOLD pile size per player per turn, and turns with no run play. See the playtest README.
